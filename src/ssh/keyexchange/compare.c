...
static void get_best_guess(struct ssh_string_s *clist_c, struct ssh_string_s *clist_s, char *name, unsigned int size)
{
    ...
    logoutput("get_best_guess: compare client (len=%i) %s", strlen(list_c), list_c);
    logoutput("get_best_guess: compare server (len=%i) %s", strlen(list_s), list_s);
...
int compare_msg_kexinit(struct ssh_connection_s *connection)
{
    ...
    */
	...
	init_ssh_string(&clist_client[i]);
	init_ssh_string(&clist_server[i]);

	len_client=read_ssh_string(pos_client, left_client, &clist_client[i]);
	// logoutput("compare_msg_kexinit: left: %i len client %i", left_client, clist_client[i].len);

	if (len_client==0) {

	    logoutput("compare_msg_kexinit: error reading commalist from client kexinit message");
	    goto error;

	}

	len_server=read_ssh_string(pos_server, left_server, &clist_server[i]);
	// logoutput("compare_msg_kexinit: left: %i len server %i", left_server, clist_server[i].len);

	if (len_server==0) {

	    logoutput("compare_msg_kexinit: error reading commalist from server kexinit message");
	    goto error;
