...
void process_sftp_subsystem_init(struct sftp_payload_s *payload)
{
    ...
    set_sftp_subsystem_process_payload(sftp, "error");

    if (payload->len<4) {

        logoutput("process_sftp_init: error size less than 4 (%i)", payload->len);
        goto disconnect;
	...
	logoutput("process_sftp_init: init version already received");
        goto disconnect;

    }

    sftp->flags |= SFTP_SUBSYSTEM_FLAG_VERSION_RECEIVED;
    clientversion=get_uint32(&buffer[0]);
