...
char *get_pk_identity_source(struct pk_identity_s *identity)
{
    if (identity->source == PK_IDENTITY_SOURCE_OPENSSH_LOCAL) return "local openssh";
    return "unknown";
}
...
int read_key_param(struct pk_identity_s *identity, struct ssh_key_s *key)
{
    ...
    char buffer[len];
    struct ssh_string_s keymaterial=SSH_STRING_INIT;
    unsigned int error=0;
