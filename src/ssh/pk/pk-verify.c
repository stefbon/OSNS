...
static int read_pk_signature_params(gcry_mpi_t *tmp, unsigned int len, struct ssh_string_s *sig)
{
    gcry_error_t err = 0;
    char *pos=sig->ptr;
...
int verify_sig_rsa(struct ssh_key_s *key, unsigned char *buffer, unsigned int size, struct ssh_string_s *sig, const char *hashname)
{
...
int verify_sig_dss(struct ssh_key_s *key, unsigned char *buffer, unsigned int size, struct ssh_string_s *sig, const char *hashname)
{
    ...
    if (read_pk_signature_params(&m_sig[0], 2, sig)==-1) {
...
int verify_sig_ecc(struct ssh_key_s *key, unsigned char *buffer, unsigned int size, struct ssh_string_s *sig, const char *hashname)
{
    int algo = gcry_md_map_name((hashname) ? hashname : "sha1");
    gcry_error_t err = 0;
...
int verify_sig(struct ssh_key_s *key, unsigned char *buffer, unsigned int size, struct ssh_string_s *sig, const char *hashname)
{
	...
	return verify_sig_rsa(key, buffer, size, sig, hashname);

    case SSH_PKALGO_SCHEME_DSS:

	return verify_sig_dss(key, buffer, size, sig, hashname);

    case SSH_PKALGO_SCHEME_ECC:

	return verify_sig_ecc(key, buffer, size, sig, hashname);
