...
static int read_keys_openssh_key_v1(char *buffer, unsigned int size, struct ssh_pkalgo_s *pkalgo, struct ssh_string_s *pk, struct ssh_string_s *sk)
{
    ...
    struct ssh_string_s cipher=SSH_STRING_INIT;
    struct ssh_string_s kdf=SSH_STRING_INIT;
    struct ssh_string_s kdfoptions=SSH_STRING_INIT;
    unsigned int count;
    int result=-1;
    struct ssh_string_s esk=SSH_STRING_INIT; /* encrypted secret key*/
