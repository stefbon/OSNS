...
unsigned int get_nbits_ssh_mpint(struct ssh_mpint_s *mp)
{
	...
	unsigned int nbits=gcry_mpi_get_nbits(mp->lib.mpi);
...
{
    /**/
    logoutput("power_modulo_ssh_mpint: result %s %i", (result->lib.mpi)?"def":"nul", get_nbits_ssh_mpint(result));
    logoutput("power_modulo_ssh_mpint: b %s %i", (b->lib.mpi)?"def":"nul", get_nbits_ssh_mpint(b));
    logoutput("power_modulo_ssh_mpint: e %s %i", (e->lib.mpi)?"def":"nul", get_nbits_ssh_mpint(e));
    logoutput("power_modulo_ssh_mpint: m %s %i", (m->lib.mpi)?"def":"nul", get_nbits_ssh_mpint(m));
...
{
    return gcry_mpi_cmp_ui(a->lib.mpi, l);
}
...
void add_ssh_mpint(struct ssh_mpint_s *w, struct ssh_mpint_s *u, const char *what, unsigned long n)
{
	...
	gcry_mpi_add_ui(w->lib.mpi, u->lib.mpi, n);

    } else if (strcmp(what, "sub")==0) {

	gcry_mpi_sub_ui(w->lib.mpi, u->lib.mpi, n);

    }
}

void mul_ssh_mpint(struct ssh_mpint_s *result, struct ssh_mpint_s *u, const char *what, unsigned long n)
{

    if (strcmp(what, "mul")==0) {

	gcry_mpi_mul_ui(result->lib.mpi, u->lib.mpi, n);

    } else if (strcmp(what, "div")==0) {
	gcry_mpi_t tmp=gcry_mpi_set_ui(NULL, n);

	logoutput("mul_ssh_mpint: div");
	gcry_mpi_div(result->lib.mpi, NULL, u->lib.mpi, tmp, 0);
