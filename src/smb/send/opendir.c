...
static void _smb_opendir_cb(struct smb2_context *smb2, int status, void *command_data, void *private_data)
{
	...
	logoutput_warning("_smb_opendir_cb: failed to get smb dir %i - %s - %s", status, strerror(status), smb2_get_error(smb2));
    ...
    logoutput_warning("_smb_opendir_cb: original smb request not found (timedout..?)");
    remove_list_element(&data->list);
    free(data);
...
int send_smb_opendir_ctx(struct context_interface_s *interface, struct smb_request_s *smb_r, char *path, struct smb_data_s *data)
{
    ...
    return smb2_opendir_async(smb2, path, _smb_opendir_cb, (void *) data);
}
...
{
    struct smb2_context *smb2=get_smb2_context_smb_interface(interface);
    struct smb2dir *dirh=(struct smb2dir *) ptr;
    struct smb2dirent *de=smb2_readdir(smb2, dirh);
