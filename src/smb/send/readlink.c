...
static void _smb_readlink_cb(struct smb2_context *smb2, int status, void *command_data, void *private_data)
{
	...
	logoutput_warning("_smb_readlink_cb: failed to get file readlink %i - %s - %s", status, strerror(status), smb2_get_error(smb2));
    ...
    logoutput_warning("_smb_readlink_cb: original smb request not found (timedout..?)");
...
int send_smb_readlink_ctx(struct context_interface_s *interface, struct smb_request_s *smb_r, char *path, struct smb_data_s *data)
{
    ...
    return smb2_readlink_async(smb2, path, _smb_readlink_cb, (void *) data);
}
