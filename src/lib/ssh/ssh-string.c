...
static unsigned int copy_ssh_string_hlpr(char *buffer, unsigned int size, char *data, unsigned int len)
{
...
unsigned int write_ssh_string(char *buffer, unsigned int size, const unsigned char type, void *ptr)
{
	...
	return copy_ssh_string_hlpr(buffer, size, (char *)s->ptr, s->len);
	break;
    }
    case 'c' : {
	char *data=(char *) ptr;
	...
	return copy_ssh_string_hlpr(buffer, size, data, len);
	break;
    }
    case 'l' : {
