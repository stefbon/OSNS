...
static void queue_service_found(unsigned int code, unsigned int method, unsigned int depends, struct host_address_s *host, struct service_address_s *service)
{
	...
	memcpy(&queue->host, host, sizeof(struct host_address_s));
	memcpy(&queue->service, service, sizeof(struct service_address_s));
...
static int compare_service_address(struct service_address_s *a, struct service_address_s *b)
{
    ...
    } else if (a->type==_SERVICE_TYPE_PORT) {

	return (a->target.port.nr==b->target.port.nr && a->target.port.type==b->target.port.type) ? 0 : -1;
...
static unsigned int add_net_service_unlocked(unsigned int code, unsigned int method, struct host_address_s *hosta, struct service_address_s *servicea, unsigned char queued, unsigned int *error)
{
    ...
    *error=0;
	...
	if (compare_host_address(&host->host, hosta)==0) break;
	...
	if (host) {
	    ...
	    memcpy(&host->host, hosta, sizeof(struct host_address_s));
	...
	if (compare_service_address(&service->service, servicea)==0) break;
	list=get_next_element(list);
	service=NULL;
	...
	if (service) {
	    ...
	    service->host=host;
	    memcpy(&service->service, servicea, sizeof(struct service_address_s));
	    ...
	    logoutput("add_net_service_unlocked: added service port %i", servicea->target.port.nr);
...
void get_net_services(struct timespec *since, process_new_service cb, void *ptr)
{
	...
	result=add_net_service_unlocked(queue->code, queue->method, &queue->host, &queue->service, 1, &error);
...
static void start_get_service_thread(struct timespec *since)
{
	...
	memcpy(&process_services.since, since, sizeof(struct timespec));
	work_workerthread(NULL, 0, process_services_thread, NULL, NULL);
...
static unsigned int add_net_service_common(unsigned int code, char *hostname, char *ip, unsigned int port, unsigned int method, unsigned int depends)
{
	...
	memset(&host, 0, sizeof(struct host_address_s));
	...
	if (method==DISCOVER_METHOD_AVAHI) {
	    ...
	    set_host_address(&host, NULL, ip, NULL);

	} else {

	    set_host_address(&host, hostname, ip, NULL);

	}

	service.type=_SERVICE_TYPE_PORT;
	service.target.port.nr=port;
	...
	queue_service_found(code, method, depends, &host, &service);
