...
static int get_write_access_ctx(struct service_context_lock_s *servicelock, struct list_lock_s *lock, const char *what)
{
    ...
    if (lock->value & SIMPLE_LIST_LOCK_WRITE) {
	...
	if (lock->threadidw==pthread_self()) {
	    ...
	    if ((lock->value & SIMPLE_LIST_LOCK_PREWRITE)==0) {

		lock->value |= SIMPLE_LIST_LOCK_PREWRITE;
		lock->threadidpw=pthread_self();
		*p_lockflags |= SIMPLE_LIST_LOCK_PREWRITE;
    ...
    } else if (lock->value & SIMPLE_LIST_LOCK_PREWRITE) {

	if (lock->threadidpw==pthread_self()) {

	    /* pre write owned by this thread */

	    if (lock->value <= (SIMPLE_LIST_LOCK_PREWRITE | SIMPLE_LIST_LOCK_WRITE)) {

		/* no readers */

		lock->value |= SIMPLE_LIST_LOCK_WRITE;
		lock->value &= ~ SIMPLE_LIST_LOCK_PREWRITE;
		lock->threadidw=lock->threadidpw;
		lock->threadidpw=0;
		*p_lockflags |= SIMPLE_LIST_LOCK_WRITE;
		*p_lockflags &= ~ SIMPLE_LIST_LOCK_PREWRITE;
...
static int get_read_access_ctx(struct service_context_lock_s *servicelock, struct list_lock_s *lock, const char *what)
{
    ...
    if ((lock->value & (SIMPLE_LIST_LOCK_PREWRITE | SIMPLE_LIST_LOCK_WRITE))==0) {

	lock->value += SIMPLE_LIST_LOCK_READ;
	*p_lockflags=SIMPLE_LIST_LOCK_READ;
	return 1;
...
static void release_access_ctx(struct service_context_lock_s *servicelock, struct list_lock_s *lock, const char *what)
{
    ...
    if (lockflag & SIMPLE_LIST_LOCK_READ) {

	if (lock->value >= SIMPLE_LIST_LOCK_READ) lock->value -= SIMPLE_LIST_LOCK_READ;
	lockflag &= ~SIMPLE_LIST_LOCK_READ;

    }

    if (lockflag & SIMPLE_LIST_LOCK_PREWRITE) {

	if (lock->threadidpw==pthread_self()) {

	    lock->value &= ~SIMPLE_LIST_LOCK_PREWRITE;
	    lockflag &= ~SIMPLE_LIST_LOCK_PREWRITE;

	}

    }

    if (lockflag & SIMPLE_LIST_LOCK_WRITE) {

	if (lock->threadidw==pthread_self()) {

	    lock->value &= ~SIMPLE_LIST_LOCK_WRITE;
	    lockflag &= ~SIMPLE_LIST_LOCK_WRITE;
