...
static struct workerthread_s *create_workerthread(struct workerthreads_queue_s *queue, struct generic_error_s *error)
{
    struct workerthread_s *thread=NULL;
    int result;
    pthread_attr_t attr;
	...
	set_generic_error_system(error, result, __PRETTY_FUNCTION__);
	free(thread);
	return NULL;
    ...
    result=pthread_create(&thread->threadid, &attr, (void *) process_job, (void *) thread);
    pthread_attr_destroy(&attr);
...
void work_workerthread(void *ptr, int timeout, void (*cb) (void *data), void *data, struct generic_error_s *error)
{
    struct workerthreads_queue_s *queue=NULL;
    struct threadjob_s *job=NULL;
	...
	pthread_mutex_unlock(&queue->mutex);
	set_generic_error_system(error, EPERM, __PRETTY_FUNCTION__);
	return;
    ...
    */
    ...
    /* maximum number of threads or unable to create a thread: put job on queue */
...
void start_default_workerthreads(void *ptr)
{
	...
	work_workerthread(ptr, 0, start_workerthread_log, NULL, NULL);
